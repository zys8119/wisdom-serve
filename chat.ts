...
export const chatOld = async function (req, res, { userInfo: info }) {
  try {
    ...
    const taskQueue = [];
    // 获取内置assistant 信息
    const { results: assistantResults } = await this.$DB_$chat.query(
      chatSqls.query_system_assistant,
      [info.chat_id]
    );
    assistantResults.forEach((element) => {
      messages.push({
        role: element.role || "system",
        content: element.message || 0,
      });
    });
    const { results } = await this.$DB_$chat.query(
      chatSqls.query_chat_history_by_chat_id,
      [info.chat_id]
    );
    ...
    const close = async () => {
      if (!isAdd) {
        ...
        // 更新历史标题
        const { results } = await this.$DB_$chat.query(
          chatSqls.query_chat_history_need_update,
          [info.chat_id]
        );
        if (results?.[0]) {
          let newTitle = "";
          const response: any = await ollama.chat({
            ...
            messages: messages.concat([
              { role: "assistant", content: systemMessages },
              {
                role: "user",
                content:
                  "以上对话请总结出一个标题，请纯文字的形式返回一句话，且不要输出markdown格式，尽可能的简洁明了，语句通顺",
              },
            ]),
          });
          for await (const part of response) {
            if (part.done) {
              await this.$DB_$chat.query(chatSqls.update_chat_history_title, [
                newTitle,
                info.chat_id,
              ]);
            } else {
    ...
    this.response.on("close", async () => {
      ...
      if (isNeedUpdateTitle) {
        this.response.write(
          `data: ${JSON.stringify({ isNeedUpdateTitle: true })}\n\n`
        );
      }
...
export const getChatToken = async function (
  ...
  try {
    ...
    const {
      results: [chatSession],
    } = await this.$DB_$chat.query(sqls.query_history_details, [
      aiAssistantChatId,
    ]);
    if (!chatSession) {
      return this.$error("该会话不存在！", {
...
export const history = async function (req, res, { userInfo: { uid, tid } }) {
  try {
    const sqls = sql("./chat.sql");
    this.$success(
      (await this.$DB_$chat.query(sqls.query_history, [uid, tid])).results
    );
  } catch (err) {
    console.error(err);
    this.$error(err.err || err.message);
...
export const createHistory = async function (
  ...
  try {
    ...
    await this.$DB_$chat.query(sqls.createHistory, [
      uuid,
      uid,
      tid,
      title,
